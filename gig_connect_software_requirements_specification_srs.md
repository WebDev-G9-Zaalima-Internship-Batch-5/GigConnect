...
- **Registered Client** — Posts gigs, invites freelancers, makes payments, rates freelancers.
- **Registered Freelancer** — Creates profile, applies to gigs, receives payments, rates clients.
- **Admin** — Manages users, gigs, disputes, and content moderation.

### 2.3 Operating Environment
- Backend: Node.js (LTS), Express, MongoDB (Atlas or self-hosted), Socket.IO
- Frontend: React (Vite or Create React App), Tailwind or CSS framework
...
- **FR-01:** Users can register as a *Client* or *Freelancer* with email verification.
...
- **FR-22:** Aggregate rating displayed on freelancer profile.
...
- **NFR-10:** Support horizontal scaling: stateless backend servers, shared session/DB, Redis for pub/sub and caching.
...
- **NFR-16:** Codebase modularity, unit/integration tests coverage (goal: 70%+ for critical modules).
- **NFR-17:** CI/CD pipeline with automated tests and linting.
...
- Payment providers: Razorpay or Stripe.
- Email provider: SendGrid / Mailgun (for transactional emails).
- Optional: SMS gateway for OTPs.
- CDN / Cloud storage for uploads (AWS S3, Cloudinary).

---

## 6. Data Model (Overview)
Provide an initial schema overview (MongoDB collections):

- `users` (common fields): _id, name, email, passwordHash, role, location { city, coords }, profileId, createdAt, status
- `profiles` (freelancer details): userId, headline, bio, skills[], rates {hourly, fixed}, portfolio[], availability, ratingAggregate, reviewsCount
- `gigs`: _id, clientId, title, description, requiredSkills[], location{coords, radius}, budget{min,max,type}, milestones[], status, applicants[]
- `applications`: _id, gigId, freelancerId, proposalText, proposedPrice, status, createdAt
- `messages`: _id, participants[], messages[{senderId, text, attachments, timestamp, read}] (or normalized per message doc)
- `reviews`: _id, gigId, reviewerId, revieweeId, rating, text, createdAt
- `transactions`: _id, gigId, payerId, payeeId, amount, status, providerTxnId, createdAt

(Include indexes: users.email(unique), profiles.skills(text), gigs.location(2dsphere), gigs.clientId, transactions.providerTxnId)
...
- `POST /api/v1/auth/register` — register user (role)

---

## 10. Deployment & Maintenance
- CI/CD pipeline to run tests and deploy staging/production.
- Use environment-based configuration for keys and provider credentials.
- Backups for the database and periodic restore drills.

---

## 11. Roadmap / Future Enhancements
- Mobile apps (iOS/Android) — Phase 2
- In-app video calls / interviews
- Proposal templates & bidding analytics
- Subscription plans and promoted listings
- Localized currency & multi-country support

---

## 12. Appendix
- Wireframe sketches (attach separately)
- Sample email templates (attach separately)
- Glossary of terms
